...
-(void)parseOUI {
    ...
    NSString *filepath = [[NSBundle mainBundle] pathForResource:@"oui" ofType:@"txt"];
    NSError *error;
    ...
    //NSError *error = NULL;
    NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:@"[A-F0-9]{2}-[A-F0-9]{2}-[A-F0-9]{2}\\s*\\(hex\\)\\s*[A-Za-z\\.\\, \\-]+"
    ...
    BOOL wrote = [macDictionary writeToFile:path atomically:YES];
    
    NSString *result = wrote ? [NSString stringWithFormat:@"Successfull. You can find the plist at this path: %@",path]: @"Not successfull";
    NSLog(@"%@",result);
